...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        #flash attention -> uses online calculation of softmax -> less memory access
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        #init params
        self.apply(self._init_weights) #iterates each module and applies the function

    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            std = 0.02
            if hasattr(module, "NANOGPT_SCALE_INIT"):
                std += (2 * self.config.n_layer) ** -0.5 # each module has two blocks
            torch.nn.init.normal_(module.weight, mean = 0.0, std = 0.02) #usually would like something that grows with the model
    ...
    def forward(self, idx, targets = None):
        B, T = idx.size()
        assert T <= self.config.block_size, f"Cannot forward sequence of length {T}, block"
...
class DataLoaderLite:
    def __init__(self, B, T):
        ...
        with open("original_transformer/input.txt", "r") as f:
            text = f.read()
